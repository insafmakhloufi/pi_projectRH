...
public class QuizController implements Initializable {
    ...
    private void finishQuiz() {
        ...
        int candidatId = Session.getCurrentUser() != null
                ? Session.getCurrentUser().getId()
                : -1;
        if (candidatId > 0) {
            TestResultat tr = new TestResultat();
            tr.setTestId(test.getId());
            tr.setCandidatId(candidatId);
            tr.setScore(score);
            tr.setMaxScore(Math.max(1, maxScore));
            tr.setCorrect(correct);
            tr.setWrong(wrong);
            tr.setUnanswered(unanswered);
            tr.setTimeSpentSec(spent);
            tr.setPercent(result.percent());
            tr.setMention(result.gradeMessage());
            tr.setDatePassage(LocalDateTime.now());
            new TestResultatService().saveResultat(tr);

            new TestService().setVisible(test.getId(), false);
        }
