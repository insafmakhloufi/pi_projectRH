...
public class TestListController implements Initializable {
    ...
    private void handleModifier() {
        ...
        try {
            ...
            test.setVisible(selectedTest.isVisible());
    ...
    private void handleVoirResultats() {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/testQuiz/fxml/test_resultats.fxml"));
            Parent view = loader.load();
            navigateInParent(view);
        } catch (IOException e) {
            showError("Erreur", "Impossible de charger les resultats.");
            e.printStackTrace();
        }
    }
    ...
    private VBox createCard(Test test) {
        ...
        Button toggleVisible = new Button(test.isVisible() ? "🟢 Visible" : "⚫ Caché");
        toggleVisible.getStyleClass().add("btn-secondary");
        toggleVisible.setOnAction(e -> {
            if (testService.toggleVisible(test.getId())) {
                loadTests();
            }
        });
        ...
        HBox actions = new HBox(8, edit, toggleVisible, details);
