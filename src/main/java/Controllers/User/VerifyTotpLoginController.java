...
public class VerifyTotpLoginController {
    ...
    public void handleVerify() {
        ...
        int userId = TotpLoginContext.getUserId();
        if (userId <= 0) {
            codeError.setText("Missing login context. Please login again.");
            return;
        }
        ...
        boolean ok = totpService.verifyCode(userId, code);
        if (!ok) {
        ...
        if (user == null) {
            codeError.setText("User not found");
            return;
        }

        TotpLoginContext.clear();
