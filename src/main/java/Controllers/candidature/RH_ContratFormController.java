...
public class RH_ContratFormController {
    ...
    public void initContrat(Candidature c) {
        ...
        Integer offreId = c.getOffreId();
        if (offreId == null) {
            tfSecteur.setText("");
            tfLocalisation.setText("");
            tfNomEntreprise.setText("");
            lblError.setText("Offre introuvable: candidature sans offre_id.");
            return;
        }

        OffreEmploi offre = offreService.getOffreByIdDetailed(offreId);
        if (offre == null) {
            tfSecteur.setText("");
            tfLocalisation.setText("");
            tfNomEntreprise.setText("");
            lblError.setText("Offre introuvable (id=" + offreId + ").");
            return;
        }

        if (tfSecteur != null) {
            String s = (offre.getSecteur() != null) ? offre.getSecteur().getNom() : "";
            tfSecteur.setText(s != null ? s : "");
        }
        if (tfLocalisation != null) {
            String loc = "";
            if (offre.getLocalisation() != null) {
                String ville = offre.getLocalisation().getVille() != null ? offre.getLocalisation().getVille().trim() : "";
                String pays = offre.getLocalisation().getPays() != null ? offre.getLocalisation().getPays().trim() : "";
                loc = (!ville.isEmpty() && !pays.isEmpty()) ? (ville + ", " + pays) : (!ville.isEmpty() ? ville : pays);
            }
            tfLocalisation.setText(loc);
        }
        if (tfNomEntreprise != null) {
            String ent = offre.getNomEntreprise();
            if (ent == null || ent.isBlank()) {
                ent = (offre.getIdEntreprise() != null) ? ("Entreprise #" + offre.getIdEntreprise()) : "";
            }
            tfNomEntreprise.setText(ent);
        }

        if (offre.getTypeContrat() != null && cbTypeContrat != null) {
            String t = offre.getTypeContrat().getNom();
            if (t != null && !t.isBlank()) {
                if (!cbTypeContrat.getItems().contains(t)) {
                    cbTypeContrat.getItems().add(t);
                }
                cbTypeContrat.setValue(t);
            }
        }
        if (offre.getTypeEmploi() != null && cbTypeEmploi != null) {
            String te = offre.getTypeEmploi().getNom();
            if (te != null && !te.isBlank()) {
                if (!cbTypeEmploi.getItems().contains(te)) {
                    cbTypeEmploi.getItems().add(te);
                }
                cbTypeEmploi.setValue(te);
            }
        }

        if (cbModeTravail != null && offre.getTeletravail() != null) {
            boolean teletravail = Boolean.TRUE.equals(offre.getTeletravail());
            if (teletravail) {
                if (cbModeTravail.getItems().contains("Remote")) {
                    cbModeTravail.setValue("Remote");
                }
            } else {
                if (cbModeTravail.getItems().contains("Présentiel")) {
                    cbModeTravail.setValue("Présentiel");
                }
            }
        }
