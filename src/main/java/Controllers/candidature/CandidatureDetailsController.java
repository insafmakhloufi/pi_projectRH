...
public class CandidatureDetailsController {
    ...
    private String subtitleOf() {
        String offre = getOffreTitreSafe();
        String deg = safe(candidature.getHighest_degree());
        String inst = safe(candidature.getInstitution());
        String base;
        if (!deg.isBlank() && !inst.isBlank()) base = deg + " • " + inst;
        else if (!deg.isBlank()) base = deg;
        else if (!inst.isBlank()) base = inst;
        else base = "";

        if (!offre.isBlank()) {
            if (base.isBlank()) return "Offre : " + offre;
            return base + " • Offre : " + offre;
        }
        return base;
    }

    private String getOffreTitreSafe() {
        if (candidature == null || candidature.getOffreId() == null) return "";
        try {
            OffreEmploi o = offreService.getOffreById(candidature.getOffreId());
            if (o == null || o.getTitre() == null) return "";
            return o.getTitre().trim();
        } catch (Exception ignored) {
            return "";
        }
    }
