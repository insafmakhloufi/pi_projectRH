...
public class FormationPurchaseService {
    ...
    public FormationPurchaseService() {
        ...
        Stripe.apiKey = getenvOrDefault("STRIPE_SECRET_KEY", "");
    ...
    public String createCheckoutSessionUrl(String formationTitle, double price, int formationId, int userId, String promoCode) throws StripeException {
        if (hasPurchased(userId, formationId)) {
            throw new IllegalStateException("Formation dÃ©jÃ  payÃ©e");
        }
    ...
    private void sendPromoCodeEmailAndMarkSent(int userId, String promoCode) throws java.sql.SQLException {
        ...
        String html = "" +
                ...
                "    <div style='font-size:16px;font-weight:700;margin-bottom:8px;'>FÃ©licitations !</div>" +
                "    <div style='font-size:13px;line-height:1.5;color:#374151;'>" +
                "      Vous avez effectuÃ© " + PROMO_EVERY_N_PURCHASES + " achats de formations. Voici votre code promo de " + PROMO_DISCOUNT_PERCENT + "% sur votre prochain achat :" +
                "    </div>" +
                ...
                "      Code Ã  usage unique, valable sur la prochaine formation payante." +
                "    </div>" +
                "  </div>" +
                "</div>" +
                "</body></html>";
