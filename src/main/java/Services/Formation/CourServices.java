...
public class CourServices implements IcourServices {
    ...
    public void ajouterCour(Cour c) {
        String req = "INSERT INTO Cour (formation_id, nom_formateur, `complexité`, description, origine, `duré`, nb_chapitres) VALUES (?,?,?,?,?,?,?)";
        try {
    ...
    public void modifierCour(Cour c) {
        String req = "UPDATE Cour SET nom_formateur = ?, `complexité` = ?, description = ?, origine = ?, `duré` = ?, nb_chapitres = ? WHERE id = ?";

        if (c == null) {
            throw new IllegalArgumentException("Cour ne doit pas être null");
        }
        if (c.getId() <= 0) {
    ...
    public Cour getCourById(int id) {
        String req = "SELECT id, formation_id, nom_formateur, `complexité` AS complexite, description, origine, `duré` AS dure, nb_chapitres FROM Cour WHERE id = ?";
        try {
            PreparedStatement ste = con.prepareStatement(req);
            ste.setInt(1, id);
    ...
    public List<Cour> afficherCours() {
        ...
        String req = "SELECT id, formation_id, nom_formateur, `complexité` AS complexite, description, origine, `duré` AS dure, nb_chapitres FROM Cour";
        try {
    ...
    public List<Cour> afficherCoursParFormation(int formationId) {
        List<Cour> cours = new ArrayList<>();
        String req = "SELECT id, formation_id, nom_formateur, `complexité` AS complexite, description, origine, `duré` AS dure, nb_chapitres FROM Cour WHERE formation_id = ?";
        try {
            PreparedStatement ste = con.prepareStatement(req);
    ...
    public float sommeDureeCoursParFormation(int formationId) {
        String req = "SELECT COALESCE(SUM(`duré`), 0) AS total FROM Cour WHERE formation_id = ?";
        try {
            PreparedStatement ste = con.prepareStatement(req);
            ste.setInt(1, formationId);
            ResultSet rs = ste.executeQuery();
    ...
    public float sommeDureeCoursParFormationSansCour(int formationId, int courId) {
        String req = "SELECT COALESCE(SUM(`duré`), 0) AS total FROM Cour WHERE formation_id = ? AND id <> ?";
        try {
            PreparedStatement ste = con.prepareStatement(req);
            ste.setInt(1, formationId);
            ste.setInt(2, courId);
