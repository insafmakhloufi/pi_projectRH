...
public class UserService implements IUserService {
    ...
    public List<String> getEmailsByRole(String role) {
        ...
        String sql = "SELECT email FROM user WHERE LOWER(role) = LOWER(?)";
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            System.out.println("[ERROR] Error fetching emails by role");
            e.printStackTrace();
        }
    ...
    public List<User> getUsersByRole(String role) {
        ...
        String sql = "SELECT id, email, role FROM user WHERE LOWER(role) = LOWER(?)";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, role.trim());
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    users.add(new User(id, null, email, null, null, null, r));
                }
            }
        } catch (SQLException e) {
            System.out.println("[ERROR] Error fetching users by role");
            e.printStackTrace();
        }
