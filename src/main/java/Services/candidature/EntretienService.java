...
public class EntretienService implements IEntretienServices {
    ...
    public List<Entretien> afficherEntretiensPourManager(int managerRHId) {

        List<Entretien> list = new ArrayList<>();
        String sql = "SELECT e.* " +
                "FROM entretien e " +
                "JOIN candidature c ON e.id_candidature = c.IDCandidat " +
                "JOIN offreemlpoi o ON c.offre_id = o.id " +
                "JOIN entreprise en ON o.id_entreprise = en.id_entreprise " +
                "WHERE en.id_user = ? " +
                "ORDER BY e.date DESC, e.heure DESC, e.id_entretien DESC";
