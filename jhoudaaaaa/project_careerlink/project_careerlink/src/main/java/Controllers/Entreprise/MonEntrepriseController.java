            ...
            if (filialesListContainer != null) {
                ...
                if (locStats.isEmpty()) {
                    ...
                    for (Map.Entry<String, Integer> entry : sorted) {
                        String ville = entry.getKey() != null ? entry.getKey() : "";
                        int count = entry.getValue() != null ? entry.getValue() : 0;

                        // Ligne compacte: ville + compteur
                        HBox row = new HBox(8);
                        row.setAlignment(Pos.CENTER_LEFT);
                        row.getStyleClass().add("filiale-compact");

                        Label villeLabel = new Label(ville);
                        villeLabel.getStyleClass().add("filiale-ville");
                        HBox.setHgrow(villeLabel, Priority.ALWAYS);

                        Label countLabel = new Label(String.valueOf(count));
                        countLabel.getStyleClass().add("filiale-count");

                        row.getChildren().addAll(villeLabel, countLabel);
                        filialesListContainer.getChildren().add(row);
                    }
                }
            }
        } catch (Exception e) {
