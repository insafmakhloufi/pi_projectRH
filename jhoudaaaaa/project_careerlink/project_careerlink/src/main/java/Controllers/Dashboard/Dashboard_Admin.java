...
import Utils.EntrepriseRequiredCheck;
...
public class Dashboard_Admin {
    ...
    private void initialize() {
        ...
        // Vérifier si l'utilisateur MANAGER_RH a une entreprise après que la scène soit attachée
        Platform.runLater(() -> {
            if (sidebar != null && sidebar.getScene() != null) {
                Stage currentStage = (Stage) sidebar.getScene().getWindow();
                if (currentStage != null) {
                    boolean hasEntreprise = EntrepriseRequiredCheck.checkAndRedirect(currentStage);
                    if (!hasEntreprise) {
                        // Redirection effectuée vers le formulaire entreprise
                    }
                }
            }
        });
    ...
    private void openCandidatures(ActionEvent e) {
        if (!checkEntrepriseRequired()) return;
        setActiveFromEvent(e);
        loadCenter("/candidaturefxml/RH_Candidatures.fxml");
    }

    private boolean checkEntrepriseRequired() {
        if (!EntrepriseRequiredCheck.hasEntreprise()) {
            if (sidebar != null && sidebar.getScene() != null) {
                Stage currentStage = (Stage) sidebar.getScene().getWindow();
                EntrepriseRequiredCheck.checkAndRedirect(currentStage);
            }
            return false;
        }
        return true;
    ...
    private void openPublications(ActionEvent e) {
        if (!checkEntrepriseRequired()) return;
        setActiveFromEvent(e);
        loadCenter("/views/event_affichage_backend.fxml");
    }
