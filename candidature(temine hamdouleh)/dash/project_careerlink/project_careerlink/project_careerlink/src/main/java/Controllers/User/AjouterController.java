...
public class AjouterController {
    ...
    private boolean validatePhoneBasic(String phone) {
        ...
        if ("Phone already exists".equals(phoneError.getText())) {
            phoneError.setText("");
        } else if (!phoneError.getText().isEmpty() && !"Phone must be 8-15 digits".equals(phoneError.getText())) {
            phoneError.setText("");
        }
        return true;
    }
    ...
    public void handleSignup() {
        ...
        if (!validatePhoneBasic(phone)) valid = false;
        if (!validatePassword(password)) valid = false;
        if (!validateRole(role)) valid = false;
        if (!valid) return;

        User user = new User(name, email, phone, selectedPhotoPath, password, role);
